...
{
    public partial class JobReceiptListView : ListBaseView
    {
        //private List<OpBillListDto> _modelList = new List<OpBillListDto>();

        public JobReceiptListView()

        {
            InitializeComponent();
            
            if(JobRecPara.Challan_Required)
                this.listDateRange1.VoucherType = VoucherTypeEnum.JobReceipt;
            else
                this.listDateRange1.VoucherType = VoucherTypeEnum.JobReceiptVoucher;
            ...
            try
            {
                ...
                using (var con = new SqlConnection(db.Database.Connection.ConnectionString))
                {
                    ...
                    using (var cmd = new SqlCommand(listDateRange1.SelectedItem.SpName, con))
                    {
                        ...
                        if(!JobRecPara.Challan_Required)
                            cmd.Parameters.Add("@VTypeId", SqlDbType.Int).Value = (int)VoucherTypeEnum.JobReceiptVoucher;
                        else
                            cmd.Parameters.Add("@VTypeId", SqlDbType.Int).Value = (int)VoucherTypeEnum.JobReceipt;
        ...
        public override void DeleteRec()
        {
            ...
            try
            {
                ...
                using (var db = new KontoContext())
                {
                    ...
                    var jr = db.JobReceipts.Where(x=>x.ChallanId == _id).ToList();
            ...
            catch (Exception ex)
            {
                ...
                Log.Error(ex, "Job Receipt delete");
        ...
        public override void Print()
        {
            ...
            if (KontoView.Columns.ColumnByFieldName("Id") != null)
            {
                ...
                var frm = new DocPrintParaView(VoucherTypeEnum.JobReceipt, "Job Receipt Print", id, id, "JRB", "Id");
