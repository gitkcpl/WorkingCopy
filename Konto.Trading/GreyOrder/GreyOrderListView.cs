...
{
    public partial class GreyOrderListView : ListBaseView
    {
        

        public GreyOrderListView()
        ...
        private void ListDateRange1_GetButtonClick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (var con = new SqlConnection(db.Database.Connection.ConnectionString))
                {
                    using (var cmd = new SqlCommand(listDateRange1.SelectedItem.SpName, con))
                    {
                        ...
                        cmd.Parameters.Add("@VTypeId", SqlDbType.Int).Value = (int)VoucherTypeEnum.GreyOrder;
            ...
            {
                Log.Error(ex, "Grey Order List Error");
        ...
        public override void Print()
        {
            ...
            if (KontoView.Columns.ColumnByFieldName("Id") != null)
            {
                ...
                var frm = new DocPrintParaView(VoucherTypeEnum.SalesOrder, "Sales Order Print",id,id, "ORD", "OrdId");
                frm.ShowDialog();
