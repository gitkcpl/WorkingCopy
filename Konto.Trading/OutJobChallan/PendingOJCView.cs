...
{
    public partial class PendingOJCView : KontoForm
    {
        public int AccId { get; set; }
        TextEdit headerEdit = new TextEdit();
        GridColumn activeCol = null;
        private string GridLayoutFileName = KontoFileLayout.OJC_Pending_Detail;
        public PendingMillReceiptSp SelectedRow { get; set; }
        public PendingOJCView()
        {
            InitializeComponent();
        ...
        private void PendingMillIssueView_Shown(object sender, EventArgs e)
        {
            try
            {
                using(var db = new KontoContext())
                {
                    ...
                    //            (int)SpCollectionEnum.PendingOJC);
                    //if (spcol == null)
                    //{
                    //    list = db.Database.SqlQuery<PendingMillReceiptSp>(
                    //    "dbo.PendingOJC @CompanyId={0},@AccountId={1},@VoucherTypeID={2}",
            ...
            {
                Log.Error(ex, "Pending Job Challan");
