CREATE PROCEDURE [dbo].[LedgerShow]
	@companyid BIGINT=0,
	@partyid BIGINT=0,
	@fromdate INT = 0,
	@todate INT = 0,
        ...
        WHERE rs.ReportId = @reportid
              ...
              AND rs.ParameterName = 'acgroup'
	           ...
	           FROM   dbo.LedgerTrans l
					...
					LEFT OUTER JOIN dbo.AccBal acb ON acb.AccId = ac.Id
        ...
        WHERE rs.ReportId = @reportid
              ...
              AND rs.ParameterName = 'acgroup'
	           ...
	           SELECT  l.CompanyID,
	                  l.AccountID,
	                  (
							dbo.keyMonthName(MONTH(L.Transdate)) + ' - ' + CAST(YEAR(L.Transdate) AS VARCHAR(4))
			   ...
			   LEFT OUTER JOIN dbo.AccBal ab ON ab.AccId = ac.Id
	           WHERE  l.CompanyID = @companyid AND l.IsDeleted = 0
