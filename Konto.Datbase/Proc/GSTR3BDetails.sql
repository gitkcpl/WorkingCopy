CREATE PROCEDURE [dbo].[GSTR3BDetails]
    @CompanyId INT =1,
	@FromDate  INT,
	@ToDate INT, 
	@YearId INT =0,
...
SELECT ac.GstIn, vc.VoucherName,p.VoucherNo, p.VDate VoucherDate,'Outward Supplies' [SupplyType],
...
SELECT ac.GstIn,vc.VoucherName,p.VoucherNo, p.VDate VoucherDate,'Inward Supplies' [SupplyType],
ISNULL(p.BillNo,p.VoucherNo) BillNo,ac.AccName Party,pd.ProductCode,pd.ProductName,isnull(pt.HsnCode,pd.HsnCode) HsnCode,
...
SELECT a.GstIn,vc.VoucherName,p.VoucherNo, p.VDate VoucherDate,'Inward Supplies' [SupplyType],
