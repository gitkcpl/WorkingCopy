CREATE PROCEDURE [dbo].[Outs_Ageing_Fifo]
    @companyid BIGINT = 0,
    @fromdate INT = 0,
    @todate INT = 0,
    @payfromdate INT = 0,
...
BEGIN
    ...
    (
              SELECT ROW_NUMBER() OVER (ORDER BY l.VoucherDate, l.AccountID) AS SrNo,
               ac.AccName AS Party,
			   l.AccountID,
               DATEDIFF(D, l.TransDate, GETDATE()) AS Days,
               l.BillNo,
               l.VoucherDate,
			   l.TransDate,
			   l.VoucherNo,
...
SELECT *
    ...
    (
              SELECT ROW_NUMBER() OVER (ORDER BY l.VoucherDate, l.AccountID) AS SrNo,
               ...
               acg.GroupName GroupName,
               AG.AccName Agent,
               CASE WHEN @nature='R' THEN  l.Credit ELSE l.Debit END  Amt
        FROM dbo.LedgerTrans l
            LEFT OUTER JOIN dbo.acc AS ac
                ON ac.Id = l.AccountId
            LEFT OUTER JOIN dbo.acgroup acg
		...
		WHERE l.IsDeleted = 0  AND ISNULL(acg.Extra1,'N') <> @nature AND l.CompanyID = @companyid
             ...
             AND ((@nature='R' AND l.Credit > 0) OR (@nature='P' AND l.Debit > 0))
			AND ( (@partyid=0 OR l.AccountID=@partyid) AND (@party='N'
                      OR EXISTS ( SELECT    1
                                  FROM      dbo.ReportPara rs
                                  WHERE     rs.ReportId = @reportid
