CREATE PROCEDURE [dbo].[IntLedger_Reports]
...
SELECT  CAST(0 AS BIT) IsSelected,xx.TransDate,
	   ...
	   xx.OpBal,
       xx.DebitAmt,
	     DATEDIFF(DAY, DATEADD(DAY, @Days, xx.transdate),convert(date,convert(varchar(8),@todate),112)) Days,
	     xx.CreditAmt,
		 xx.CreditInt,
		 xx.DebitInt,
	  ...
	  --    (xx.CreditAmt*@IntPer*(DATEDIFF(DAY, DATEADD(DAY, @Days,xx.transdate), convert(date,convert(varchar(8),@todate),112))))/(100 *@TotalDays ) CreditInt, 
       ...
       CASE
           WHEN xx.Bal > 0 THEN
               FORMAT(xx.Bal
			   +
			   ((xx.Amount *@IntPer*(DATEDIFF(DAY, DATEADD(DAY, @Days, xx.transdate),convert(date,convert(varchar(8),@todate),112))))/(100 *@TotalDays ))
			   , 'N') + ' Dr'
           ELSE
               FORMAT(-1 * (xx.Bal
			   +((xx.Amount *@IntPer*(DATEDIFF(DAY, DATEADD(DAY, @Days, xx.transdate),convert(date,convert(varchar(8),@todate),112))))/(100 *@TotalDays ))
...
(
    SELECT x.TransDate,
           ...
           x.DebitAmt,
           x.CreditAmt,
		      (x.DebitAmt*@IntPer *(DATEDIFF(DAY, DATEADD(DAY, @Days,x.transdate), convert(date,convert(varchar(8),@todate),112))))/(100 *@TotalDays ) DebitInt,
			  (x.CreditAmt*@IntPer *(DATEDIFF(DAY, DATEADD(DAY, @Days,x.transdate), convert(date,convert(varchar(8),@todate),112))))/(100 *@TotalDays ) creditInt,
           ...
           SUM(ISNULL(DebitAmt, 0) - ISNULL(CreditAmt, 0)) OVER (PARTITION BY x.AccountId ORDER BY  x.flag, x.VoucherDate, x.Id) [Amount],
		     ...
		     SUM(ISNULL((x.DebitAmt*@IntPer *(DATEDIFF(DAY, DATEADD(DAY, @Days,x.transdate), convert(date,convert(varchar(8),@todate),112))))/(100 *@TotalDays ), 0)
			  -
			   ISNULL((x.CreditAmt*@IntPer *(DATEDIFF(DAY, DATEADD(DAY, @Days,x.transdate), convert(date,convert(varchar(8),@todate),112))))/(100 *@TotalDays ), 0))
    ...
    (
        SELECT 0 AS flag,
               ...
               ar.AreaName,
               ct.CityName,
               st.StateName,0 KeyFldValue, null as Audit,null as RefRowID
        FROM dbo.AccBal op
            LEFT OUTER JOIN
            (
                SELECT ld.AccountId,
