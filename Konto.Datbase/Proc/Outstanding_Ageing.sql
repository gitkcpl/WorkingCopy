CREATE PROCEDURE [dbo].[Outstanding_Ageing] 
...
AS
     SELECT  a.AccName AS Account , l.RefType ,
              ...
              --  AG.account_name AgentName ,
				...
				CASE WHEN DATEDIFF(D, bm.VDate, GETDATE())>@range1 AND DATEDIFF(D, bm.VDate, GETDATE())<=@range2 THEN
				 CASE WHEN l.RefType = 'Debit' THEN CAST(l.BillAmt - ISNULL(adj.Pay,0) - ISNULL(Selfadj.SelfPay,0) - ISNULL(adj.Rg,0) -ISNULL(l.TdsAmt,0) + ISNULL(l.TcsAmt,0) AS NUMERIC(18,2))
	ELSE -1*CAST(l.BillAmt - ISNULL(adj.Pay,0) - ISNULL(Selfadj.SelfPay,0) - ISNULL(adj.Rg,0) -ISNULL(l.TdsAmt,0) + ISNULL(l.TcsAmt,0) AS NUMERIC(18,2))
	...
	ELSE -1*CAST(l.BillAmt - ISNULL(adj.Pay,0) - ISNULL(Selfadj.SelfPay,0) - ISNULL(adj.Rg,0) -ISNULL(l.TdsAmt,0) + ISNULL(l.TcsAmt,0) AS NUMERIC(18,2))
	 END ELSE 0 END AS AboveRangeValue,	
				...
				ELSE -1*CAST(l.BillAmt - ISNULL(adj.Pay,0) - ISNULL(Selfadj.SelfPay,0) - ISNULL(adj.Rg,0) -ISNULL(l.TdsAmt,0) + ISNULL(l.TcsAmt,0) AS NUMERIC(18,2))
				END AS PendingAmt ,
				CASE WHEN l.RefType = 'Debit' THEN CAST(CAST(l.BillAmt - ISNULL(adj.Pay,0) - ISNULL(Selfadj.SelfPay,0)  - ISNULL(adj.Rg,0) -ISNULL(l.TdsAmt,0) + ISNULL(l.TcsAmt,0) AS NUMERIC(18,2) ) AS VARCHAR(25)) + ' Dr'
				 ELSE CAST(CAST(l.BillAmt - ISNULL(adj.Pay,0) - ISNULL(Selfadj.SelfPay,0)  - ISNULL(adj.Rg,0) -ISNULL(l.TdsAmt,0) + ISNULL(l.TcsAmt,0) AS NUMERIC(18,2)) AS VARCHAR(25)) + ' Cr' END Pending,     
                l.AccountID ,
                acg.GroupName 
				FROM    dbo.BillRef l
				LEFT OUTER JOIN dbo.BillMain bm ON bm.Id = l.BillId
                LEFT OUTER JOIN dbo.acc AS a ON a.Id = l.AccountId
                LEFT OUTER JOIN dbo.acgroup acg ON acg.Id = a.GroupId
        ...
        WHERE   l.IsActive = 1 AND l.IsDeleted = 0 AND bm.IsActive = 1 AND bm.IsDeleted = 0 AND l.VoucherDate BETWEEN @fromdate AND @todate 
                AND ISNULL(acg.Nature, 'N') = @nature AND acg.Extra1 <> 'N'    AND ( ( CAST(l.BillAmt - ISNULL(adj.Pay,0) - ISNULL(Selfadj.SelfPay,0) - ISNULL(adj.Rg,0) -ISNULL(l.TdsAmt,0) + ISNULL(l.TcsAmt,0) AS NUMERIC(18,2)) <> 0 )
                    )				
