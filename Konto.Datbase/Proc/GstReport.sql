CREATE  PROCEDURE [dbo].[GstReport]
    @CompanyId INT ,
	--@TransTypeId  INT ,
	--@TransTypeId1 INT = 0,
	--@TransTypeId2 INT = 0,
...
AS
BEGIN
SELECT ac.VatTds AS Type,isnull(ac.GstIn,'')GstIn,ac.AccName , 
...
isnull(SUM(bt.NetTotal),0) AS BillAmount, 
...
LEFT OUTER JOIN (
				SELECT b.RefId , b.RefVoucherId, br.VoucherNo, br.VoucherDate FROM dbo.BtoB b 
				LEFT OUTER JOIN dbo.BillRef br ON br.BillId = b.BillId AND br.BillVoucherId = b.BillVoucherId
				WHERE b.IsActive = 1 AND b.IsDeleted = 0
				GROUP BY b.RefId , b.RefVoucherId, br.VoucherNo, br.VoucherDate
		) brr ON brr.RefId = bm.Id AND brr.RefVoucherId = bm.VoucherId
...
AND vc.VTypeId IN (12,19,24) AND (ISNULL(bm.Extra1,'SALE')='SALE')
--and ac.GstIn is not null

GROUP BY bm.id, ac.VatTds,ac.GstIn, ac.AccName, bm.VoucherNo,bm.VoucherDate ,
bk.AccName,bkk.AccName, vc.VoucherName, bm.BillType, bm.SpecialNotes,bt.HsnCode,
...
Revice.OrgBillDate ,Revice.OrgBillNo, bm.RefId,Revice.OrgStateName, brr.VoucherDate
ORDER BY bm.VoucherDate,bm.Id
--SELECT ac.VatTds AS Type,ac.GstIn,ac.AccName , 
--			revice.OrgBillDate,revice.OrgBillNo,
--		   bm.BillNo InvoiceNo,
--		   isnull(bm.RcdDate,'') InvoiceDate,
--           bm.VoucherNo AS VoucherNo,
--           ISNULL(CONVERT(Date,Convert(varchar(8),bm.VoucherDate),112),'')  VoucherDate,
...
--bt.CgstPer, bt.SgstPer, bt.IgstPer,st.StateName, bm.BillNo, bm.Rcm, bm.RcdDate ,
--bm.PortCode, bm.Rcm, bm.Itc,st.GstCode,stm.StateName,stm.GstCode,vc.VTypeId
--,Revice.OrgBillDate ,Revice.OrgBillNo, bm.RefId
--,Revice.OrgStateName
--ORDER BY bm.VoucherDate,bm.Id
END
GO
