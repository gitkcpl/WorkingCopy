IF object_id('[dbo].[PODetailList]') IS NULL 
EXEC ('CREATE PROC [dbo].[PODetailList] AS SELECT 1 AS Id') 
GO

ALTER PROCEDURE [dbo].[PODetailList]
	@FromDate int ,
	@ToDate int,
	@CompanyId int,
	@YearId int,
...
BEGIN
	...
	LEFT OUTER JOIN (
					SELECT ct.RefId,ct.MiscId,ct.RefVoucherId, SUM(ct.Pcs) Pcs ,SUM(ct.Qty) Qty FROM dbo.ChallanTrans ct
			        WHERE ct.Isdeleted = 0 AND ct.IsActive = 1 AND ct.RefVoucherId IS NOT NULL
					GROUP BY ct.RefId,ct.MiscId,ct.RefVoucherId
	) ct ON ct.RefId = ot.Id AND o.Id = ct.MiscId AND o.RefVoucherId = o.VoucherId
