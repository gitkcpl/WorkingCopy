...
namespace Konto.Data
{
    public class LedgerEff
    {
        ...
        public static void LedgerTransEntry(string type, BillModel model, KontoContext db, List<BillTransModel> trans)
        {
            ...
            if (sgst != 0)
            {
                ledger = new LedgerTransModel
                {
                    ...
                    AccountId = type == "Debit" ? 16 : 12,
                    RefAccountId = rcmy ? 24 : model.AccId,
                    Debit = type == "Debit" ? 0 : Convert.ToDecimal(sgst),
                    Credit = type == "Debit" ? Convert.ToDecimal(sgst) : 0,
                    BilllAmount = Convert.ToDecimal(totalAmt),
                    Amount = type == "Debit" ? -1 * Convert.ToDecimal(sgst) : Convert.ToDecimal(sgst)
                };
                ...
                if (rcmy)
                {
                    ledger = new LedgerTransModel
                    {
                        ...
                        AccountId = 24,
                        RefAccountId = 12,
                        Debit = type == "Credit" ? 0 : Convert.ToDecimal(sgst),
                        Credit = type == "Credit" ? Convert.ToDecimal(sgst) : 0,
                        BilllAmount = Convert.ToDecimal(totalAmt),
            ...
            if (cgst != 0)
            {
                ledger = new LedgerTransModel
                {
                    ...
                    AccountId = type == "Debit" ? 17 : 13,
                    RefAccountId = rcmy ? 25 : model.AccId,
                    Debit = type == "Debit" ? 0 : Convert.ToDecimal(cgst),
                    Credit = type == "Debit" ? Convert.ToDecimal(cgst) : 0,
                    BilllAmount = Convert.ToDecimal(totalAmt),
                    Amount = type == "Debit" ? -1 * Convert.ToDecimal(cgst) : Convert.ToDecimal(cgst)
                };
                list.Add(ledger);
            ...
            if (igst != 0)
            {
                ledger = new LedgerTransModel
                {
                    ...
                    AccountId = type == "Debit" ? 18 : 14,
                    RefAccountId = rcmy ? 26 : import ? 23 : (sez || export) && type == "Debit" ? 22 : sez && type == "Credit" ? 33 : model.AccId,
                    Debit = type == "Debit" ? 0 : Convert.ToDecimal(igst),
                    Credit = type == "Debit" ? Convert.ToDecimal(igst) : 0,
                    BilllAmount = Convert.ToDecimal(totalAmt),
                    Amount = type == "Debit" ? -1 * Convert.ToDecimal(igst) : Convert.ToDecimal(igst)
                };
            ...
            if (cess != 0)
            {
                ledger = new LedgerTransModel
                {
                    ...
                    AccountId = type == "Debit" ? 19 : 15,
                    RefAccountId = model.AccId,
                    Debit = type == "Debit" ? 0 : Convert.ToDecimal(cess),
                    Credit = type == "Debit" ? Convert.ToDecimal(cess) : 0,
                    BilllAmount = Convert.ToDecimal(totalAmt),
                    Amount = type == "Debit" ? -1 * Convert.ToDecimal(cess) : Convert.ToDecimal(cess)
                };
        ...
        public static void UpdateBalance(int _accid, KontoContext db)
        {
            ...
            if (_sum != null)
                _bal.Bal = _bal.OpBal + _sum.Debit - _bal.OpCredit;
            else
                _bal.Bal = _bal.OpBal;
