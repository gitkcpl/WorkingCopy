...
IF NOT EXISTS(SELECT 1 FROM dbo.Voucher WHERE vtypeid=54)
BEGIN
	INSERT INTO dbo.Voucher
	(
		VoucherName,SortName,VTypeId,RefVoucherId,
		IsActive,IsDeleted,CreateDate,CreateUser,IpAddress,RowId
	) Values('Bom Issue','BI',54,0,1,0,SYSDATETIME(),'Admin','NA',NEWID())
	...
	BEGIN  
		...
		VALUES(@id,@compid,'Store Issue',0,0,1,1,'{#}/{YY}',0,0,1,0,0,0,0,0,1,0,SYSDATETIME(),'Admin','NA',NEWID())
