...
{
    public partial class GroupLkpWindow : LookupForm
    {
        List<GroupLookupDto> _modelList = new List<GroupLookupDto>();
        public GroupLkpWindow()
        {
            InitializeComponent();
          
            this.GridLayoutFileName = KontoFileLayout.Group_Lookup_Layout;
            this.FormClassName = "Konto.Shared.Masters.ProductGroup.GroupIndex";
        ...
        public override void LoadData()
        {
            ...
            {
                _modelList = (from ar in _db.PGroups
                              where (!ar.IsDeleted && ar.IsActive)
                              orderby ar.GroupName
                              select new GroupLookupDto
                              {
