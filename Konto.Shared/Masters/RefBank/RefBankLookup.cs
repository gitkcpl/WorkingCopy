...
{
    public partial class RefBankLookup : LookupBase
    {
        public RefBankLookup()
        {
            InitializeComponent();
        ...
        public void SetValue()
        {
            using (var db = new KontoContext())
            {
                ...
                var model = db.RefBanks.Find(id);
                if (model != null)
                {
                    this.SelectedText = model.BankName;
                    buttonEdit1.Text = model.BankName;
                }
            }
        }
