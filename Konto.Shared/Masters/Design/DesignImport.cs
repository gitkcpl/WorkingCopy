...
{
    public partial class DesignImport : KontoForm
    {
                ...
                using (var db = new KontoContext())
                {
                    ...
                    foreach (DataRow item in _dataTable.Rows)
                    {
                       ...
                       // _imp.Id = ProdctId;
                        ...
                        if(item["PartyDesignNo"]!=null)
                            _imp.Ingr = item["PartyDesignNo"].ToString();

                        _imp.Price1 = Convert.ToDecimal(item["Packing"]);
                        _imp.Price2= Convert.ToDecimal(item["Labour"]);
                        
                        var _party = item["Party"].ToString();

                        if (!string.IsNullOrEmpty(_party))
                        {
                            var _pt = db.Accs.FirstOrDefault(x => x.AccName.ToUpper() == _party.ToUpper());
                            if (_pt != null)
                                _imp.AccId = _pt.Id;
                            else
                            {
