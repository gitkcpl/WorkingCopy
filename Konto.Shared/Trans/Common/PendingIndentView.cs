        ...
        private void PendingOrderView_Load(object sender, EventArgs e)
        {
            try
            {
                using(var db = new KontoContext())
                {
                    ...
                    List<PendingIndentDto> listDtos = new List<PendingIndentDto>();
                    //if (spcol == null)
                    //{
                        listDtos = db.Database.SqlQuery<PendingIndentDto>(
                            "dbo.pending_request_for_order @CompanyId={0},@VoucherTypeID={1}",
                      KontoGlobals.CompanyId, (int)VoucherTypeEnum.Indent).ToList();
                    //}
                    //else
                    //{
