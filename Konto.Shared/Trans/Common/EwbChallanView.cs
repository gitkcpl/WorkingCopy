...
{
    public partial class EwbChallanView : KontoForm
    {
        ...
        private async Task<bool> GenerateEwbAsync()
        {
            ...
            if (BModel.TypeId == (int) VoucherTypeEnum.Stock_Transfer)
            {
                ...
                var tbct = db.Cities.Find(tbr.CityId);

                ewbGen.toGstin = KontoUtils.Company.GstIn;
                ewbGen.toTrdName = KontoUtils.Company.CompName;
                ewbGen.toAddr1 = tbr.Address1;
                ewbGen.toAddr2 = tbr.Address2;
                ewbGen.toPlace = tbct.CityName;
                ewbGen.toPincode = Convert.ToInt32(tbr.PinCode);
                ewbGen.toStateCode = Convert.ToInt32(ct.State.GstCode);
                ewbGen.actToStateCode = Convert.ToInt32(ct.State.GstCode);
            }
            else
            {
            ...
            ewbGen.totalValue = Convert.ToDouble(TModel.Sum(x => x.Total) - TModel.Sum(x => x.Cgst) - TModel.Sum(x => x.Sgst) - TModel.Sum(x => x.Igst)
                - TModel.Sum(x => x.Cess));
