...
{
    public partial class SReturnListView : ListBaseView
    {
        //private List<OpBillListDto> _modelList = new List<OpBillListDto>();

        public SReturnListView()
                ...
                using (var _tran = db.Database.BeginTransaction())
                {
                    try
                    {
                        foreach (var rowno in drs)
                        {
                            ...
                            //var vtypeId = db.Vouchers.FirstOrDefault(k => k.Id == model.).VTypeId;
                            ...
                            if (stk != null)
                            {
                                db.StockTranses.RemoveRange(stk); // delete from stock
                            }

                            LedgerEff.DeleLedgEffect(model, db); //delete ledger
                        }
                    ...
                    {
                        _tran.Rollback();
                        Log.Error(ex, "Sales Return delete");
        ...
        public override void Print()
        {
            ...
            if (KontoView.Columns.ColumnByFieldName("Id") != null)
            {
                ...
                var frm = new DocPrintParaView(VoucherTypeEnum.SaleReturn, "Credit Note",id,id, "SALERET", "BillId");
        ...
        public override void ImportExcel()
        {
            ...
            if (MessageBox.Show("Import For Template.. ?", "Import", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                frm.FillTemplate((int)VoucherTypeEnum.SaleReturn);
                frm.ShowDialog();
            }
            else
            {
                var _exp = new SRImport();
                _exp.ShowDialog();
            }
        }
    }
