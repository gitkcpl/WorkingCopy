...
{
    public partial class ReqApproveIndex : KontoForm
    {
        int _vtypeid = 0;
        public ReqApproveIndex()
        {
            InitializeComponent();
        ...
        private void GetSimpleButton_Click(object sender, EventArgs e)
        {
            ...
            using (var db = new KontoContext())
            {
                ...
                //var spcol = null; //db.SpCollections.FirstOrDefault(k => k.Id ==
                ...
                //{
                   list = 
                      db.Database.SqlQuery<OrdAprvalDto>(
                       "dbo.GetRequsetApproveList @VouchreID={0},@CompanyId={1},@YearId={2},@FromDate={3},@ToDate={4},@OStatus={5}",
                       (int)VoucherTypeEnum.Indent, KontoGlobals.CompanyId, KontoGlobals.YearId, fdate, tdate, typeLookUpEdit.Text).ToList();
                //}
                //else
                //{
