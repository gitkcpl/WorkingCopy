...
{
    public partial class PaymentIndex : KontoMetroForm
    {
        ...
        private void SetParameter()
        {
            using (var db = new KontoContext())
            {
                var _paralists = db.CompParas.Include("SysPara")
                              .Where(x => x.SysPara.Category == "Payment" && x.CompId == KontoGlobals.CompanyId)
                             .ToList();
                ...
                foreach (var item in _paralists)
                {
                    ...
                    switch (item.ParaId)
                    {
                        case 222://payment
                            {
                                PaymentPara.Auto_Bill_Adjust = (value == "Y") ? true : false;
                                break;
                            }
                    }
                }
        ...
        private void ShowBillDetails()
        {
            ...
            var frm = new PendingBillViewWindow("P",dr.ToAccId, (int)VoucherTypeEnum.PaymentVoucher,"CREDIT",
        ...
        private void LoadData(BillModel model)
        {
            ...
            using (var _context = new KontoContext())
            {
                ...
                foreach (var _tr in _list)
                {
                    var _bl = _context.Database.SqlQuery<PendBillListDto>(
                        ...
                        (int)VoucherTypeEnum.PaymentVoucher, "CREDIT",
            ...
            this.Text = "Payment Voucher [View/Modify]";
        ...
        private void GridControl1_ProcessGridKey(object sender, KeyEventArgs e)
        {
               ...
               if (dr == null) return;
...

                {
                    if (e.KeyCode == Keys.Return)
                    {
                        ...
                        && gridView1.EditingValue != null && gridView1.EditingValue.ToString() == "Advance")
                {
                    if(e.KeyCode == Keys.Return)
                    {
                        ShowTds();
            ...
            {
                Log.Error(ex, "Payment GridControl KeyDown");
        ...
        {
            var dr = gridView1.GetRow(gridView1.FocusedRowHandle) as BankTransDto;
            if (dr == null) return;
            using(var db = new KontoContext())
            {
                var acc = db.Accs.Find(dr.ToAccId);
                if(acc!=null && acc.TdsReq.ToUpper() == "YES")
                {
                    var frm = new TdsPayView();
                    var tds = db.AccOthers.FirstOrDefault(x => x.AccId == acc.Id);
                    if (tds != null && dr.TdsAcId ==0)
                    {
                        dr.TdsPer = Convert.ToDecimal(tds.TdsPer);
                        dr.TdsAcId = Convert.ToInt32(tds.TdsAccId);
                    }
                    if(dr.Total==0)
                        dr.Total = dr.NetTotal;
                    frm.Trans = dr;
                    frm.ShowDialog();
        ...
        private void TabControlAdv1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                var _list = tabPageAdv2.Controls[0] as PaymentListView;
                _list.ActiveControl = _list.KontoGrid;
                this.Text = "Payment Voucher [View]";
                return;
            }
            if (tabControlAdv1.SelectedIndex == 1)
            {
                var _ListView = new PaymentListView();
        ...
        public override void Print()
        {
            ...
            try
            {
                ...
                frm.frmVouchers = voucherNoTextEdit.Text;
                frm.toVouchers = voucherNoTextEdit.Text;
                frm.IsPayment = true;
                if (frm.ShowDialog() != DialogResult.OK) return;
                KontoContext db = new KontoContext();
                int ReportId = 0;
                int vid = Convert.ToInt32(voucherLookup1.SelectedValue);
                if (!string.IsNullOrEmpty(frm.frmVouchers.ToString().Trim()))
                {
                    ...
                    if (frmVouchers != null && Tovoucher != null)
                    {
                ...
                if (frm.chkPrint)
                {
                    ...
                    pg1.Text = "Payment Print";
                    _tab.TabPages.Add(pg1);
                    _tab.SelectedTab = pg1;
                    frm1.TopLevel = false;
                    frm1.Parent = pg1;
                    frm1.Location = new Point(pg1.Location.X + pg1.Width / 2 - frm1.Width / 2, pg1.Location.Y + pg1.Height / 2 - frm1.Height / 2);
                    frm1.Show();// = true;
                }
                if (frm.chkFarwrd)
                {
                    ...
                    doc.Parameters["reportid"].CurrentValue = ReportId;
                    doc.Parameters["VTypeId"].CurrentValue = (int)VoucherTypeEnum.PaymentVoucher;
                    int frmdate = Convert.ToInt32(Convert.ToDateTime(voucherDateEdit.DateTime).ToString("yyyyMMdd"));
        ...
        public override void NewRec()
        {
            ...
            this.Text = "Payment Voucher [Add New]";
        ...
        public override void SaveDataAsync(bool newmode)
        {
            ...
            using (var db = new KontoContext())
            {
                using (var _tran = db.Database.BeginTransaction())
                {
                    ...
                    {
                        _tran.Rollback();
                        Log.Error(ex, "Payment Voucher Save");
                        MessageBoxAdv.Show(this, "Error While Save !!", "Exception ", ex.ToString());
