...
-- POS PURCHASE VOUCHER

IF NOT EXISTS(SELECT 1 FROM dbo.Voucher WHERE vtypeid=55)
BEGIN
	INSERT INTO dbo.Voucher
	(
		VoucherName,SortName,VTypeId,RefVoucherId,
		IsActive,IsDeleted,CreateDate,CreateUser,IpAddress,RowId
	) Values('Pos Purchase Voucher','PV',55,0,1,0,SYSDATETIME(),'Admin','NA',NEWID())
	...
	BEGIN  
		...
		VALUES(@id,@compid,'Purchase Voucher',0,0,1,1,'{#}/{YY}',0,0,1,0,0,0,0,0,1,0,SYSDATETIME(),'Admin','NA',NEWID())
...
-- Pos Sales Invoice
IF NOT EXISTS(SELECT 1 FROM dbo.Voucher WHERE vtypeid=56)
BEGIN
	INSERT INTO dbo.Voucher
	(
		VoucherName,SortName,VTypeId,RefVoucherId,
		IsActive,IsDeleted,CreateDate,CreateUser,IpAddress,RowId
	) Values('Pos Invoice','Pos',56,0,1,0,SYSDATETIME(),'Admin','NA',NEWID())
	...
	BEGIN  
		...
		VALUES(@id,@compid,'Tax Invoice',0,0,1,1,'{#}/{YY}',0,0,1,0,0,0,0,0,1,0,SYSDATETIME(),'Admin','NA',NEWID())
...
-- Stock Transfer
IF NOT EXISTS(SELECT 1 FROM dbo.Voucher WHERE vtypeid=57)
BEGIN
	INSERT INTO dbo.Voucher
	(
		VoucherName,SortName,VTypeId,RefVoucherId,
		IsActive,IsDeleted,CreateDate,CreateUser,IpAddress,RowId
	) Values('Stock Transfer','ST',57,0,1,0,SYSDATETIME(),'Admin','NA',NEWID())
	...
	BEGIN  
		...
		VALUES(@id,@compid,'Transfer Challan',0,0,1,1,'{#}/{YY}',0,0,1,0,0,0,0,0,1,0,SYSDATETIME(),'Admin','NA',NEWID())
