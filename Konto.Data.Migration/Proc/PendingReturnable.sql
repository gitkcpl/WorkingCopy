IF object_id('[dbo].[PendingReturnable]') IS NULL 
EXEC ('CREATE PROC [dbo].[PendingReturnable] AS SELECT 1 AS Id') 
GO

ALTER PROCEDURE [dbo].[PendingReturnable]
 	@CompanyId int,
	@AccountId int,
	@VoucherTypeID int,
	@ChallanType int
AS
BEGIN
	select c.VoucherNo,CONVERT(Date,Convert(varchar(8),c.VoucherDate),112) IssueDate ,c.VoucherId,
	ct.Pcs AS IssuePcs,CAST(ct.Qty as numeric(18,2)) as IssueQty,
	CAST((ct.Qty-ISNULL(pen.Qty,0))  AS numeric(18,2)) AS PendingQty,
	(ct.Pcs-ISNULL(pen.Pcs,0))  AS PendingPcs, ac.AccName AS Party,
	 ct.ColorId,
	...
	,ct.Id as TransId,c.Id ,ISNULL(pen.Qty,0.00) RcptQty,Ct.Remark
    FROM Challan c
	...
	LEFT outer join ( SELECT sum(Qty)as Qty, SUM(Pcs) AS Pcs , ct.RefId, ct.RefVoucherId FROM ChallanTrans ct
     ...
     WHERE c.ChallanType=@ChallanType 
	 AND v.VTypeId=@VoucherTypeID
	 AND c.AccId=@AccountId AND c.CompId =@CompanyId
