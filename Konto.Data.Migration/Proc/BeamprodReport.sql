...
IF object_id('[dbo].[BeamprodReport]') IS NULL 
EXEC ('CREATE PROC [dbo].[BeamprodReport] AS SELECT 1 AS Id') 
GO 

ALTER PROCEDURE [dbo].[BeamprodReport]
	@CompanyId int=1,
	@VoucherID int=0,
	@FromDate int=0,
	@ToDate int=0,
...
,p.Ply as BLength,tb.Taka Cops ,p.CopsWt,p.CartnWt,p.GrossWt,p.TareWt
	  ...
	  MM.MachineName as MachineNo,tb.TakaQty as TakaProd,p.NetWt-isnull(tb.TakaQty,0) as BalQty,
