IF object_id('[dbo].[OutwardBeamProd]') IS NULL 
EXEC ('CREATE PROC [dbo].[OutwardBeamProd] AS SELECT 1 AS Id') 
GO

ALTER  PROCEDURE [dbo].[OutwardBeamProd]
 	@CompanyId int,
	--@VoucherId int,
--	@InwardVId int,
	@ProductId INT=0,
...
BEGIN
 ...
 WHERE pt.IsActive=1 AND pt.IsDeleted=0  and isnull(pt.TakaStatus,'NA')<> 'NMRC'
 GROUP BY pt.ProdId)Iss ON p.Id = iss.ProdId
 WHERE p.IsActive=1 AND p.IsDeleted=0 and p.IsClose=0 AND p.IsOk = @IsOk --AND c.IsDeleted=0
...
and (@CompanyId=0 or p.CompId= @CompanyId)
order by p.Id
...
SELECT p.Id,p.RowId,p.TransId,p.SrNo,c.VoucherNo AS InwardNo,ac.AccName AS Weaver,p.CProductId ProductId,pd.ProductName YarnName,
 ...
 LEFT OUTER JOIN dbo.Acc ac ON ac.Id = c.AccId
 LEFT OUTER JOIN dbo.voucher v ON v.id = p.VoucherId
 WHERE p.IsActive=1 AND p.IsDeleted=0 and p.IsClose=0 AND po.IsOk = 0 --AND c.IsDeleted=0
AND (@ProductId =0 or p.ProductId=@ProductId) 
AND (@transid =0 OR p.TransId= @transid)
AND (@voucherid=0 OR p.VoucherId=@voucherid)
AND (@ptypeid=0 OR pd.PTypeId=@ptypeid)
...
AND (@ptypeid=0 OR pd.PTypeId=@ptypeid)
AND p.NetWt+ ISNULL(Iss.IssueQty,0) > 0
and (@CompanyId=0 or p.CompId= @CompanyId) --AND p.ProdStatus = 'Stock'
order by p.Id
