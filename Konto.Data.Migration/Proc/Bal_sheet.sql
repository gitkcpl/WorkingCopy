...
BEGIN
    ...
    LEFT OUTER JOIN
                ...
                INNER JOIN dbo.AccBal AS ab ON lp.AccountId = ab.AccId AND lp.CompanyId= ab.CompId
				LEFT OUTER JOIN dbo.acgroup ag ON ag.Id = ab.GroupId
				WHERE lp.IsDeleted = 0 AND ISNULL(lp.OpBill, 'N') = 'N' 
				AND lp.VoucherDate >=@finstartdate and 
				lp.VoucherDate < @fromdate 
				AND ag.Nature IN ('LIABILITIES','ASSETS')
                  AND lp.CompanyID = @companyid AND ab.YearId=@yearid --AND lp.referenceaccountid <> 0
    ...
    LEFT OUTER JOIN
				...
				lp.VoucherDate < @fromdate 
				AND ag.Nature IN ('LIABILITIES','ASSETS')
                  AND lp.CompanyID = @companyid AND ab.YearId=@yearid
						GROUP BY lp.AccountID
					) lp ON lp.AccountID = ac.Id
    LEFT OUTER JOIN dbo.acgroup ag ON ag.Id = op.GroupId
    LEFT OUTER JOIN
   ...
   INNER JOIN
    ...
    GROUP BY ag.GroupName, ag.Id,ac.AccName,ag.Nature,op.Audit,ac.Id,pre.Diff,pre.Bal, ag.BlSort
	 HAVING (@zero=1 or SUM(ISNULL(l.Bal, 0) + ISNULL(op.OpBal,0) + ISNULL(lp.Bal,0) ) <> 0)
