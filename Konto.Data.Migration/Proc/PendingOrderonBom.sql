...
IF object_id('[dbo].[PendingOrderonBom]') IS NULL 
EXEC ('CREATE PROC [dbo].[PendingOrderonBom] AS SELECT 1 AS Id') 
GO


ALTER  PROCEDURE [dbo].[PendingOrderonBom]
 	@CompanyId int,
	@AccountId int,
	@VoucherTypeID int,
...
BEGIN
	...
	ISNULL(o.AccId,0) AS DelvAccId, ad.Address1,ad.Address2,ac.AccName Party,
	...
	cast(0 as numeric(18,2)) as PendingQty,
	...
	left outer join (select sum(Qty)as Qty , ISNULL(ct.RefId,0) AS RefId, ISNULL(ct.RefVoucherId,0) AS RefVoucherId  from ChallanTrans ct
			...
			where ct.IsActive=1 and ct.IsDeleted=0 and v.VTypeId=4 and ch.IsDeleted=0
	...
	AND NOT EXISTS( SELECT 1 FROM bomtrans bt where bt.OrderTransId = ot.Id and bt.IsDeleted=0
	and bt.IsActive=1)
	AND  o.CompId = @CompanyId and ((ot.Qty-isnull(ct.Qty,0)) >0)
