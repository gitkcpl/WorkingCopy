...
BEGIN
	 ...
	 br.BillVoucherId,CASE WHEN v.VTypeId = 12  THEN br.VoucherNo else br.BillNo END AS BillNo,
...
LEFT JOIN (
			SELECT SUM(CASE WHEN b.TransType IN ('Payment','SInvoice','PInvoice') THEN b.Amount + ISNULL(b.Adla1,0) + ISNULL(b.Adla2,0) + ISNULL(b.Adla3,0) + ISNULL(b.Adla4,0) + ISNULL(b.Adla5,0) + ISNULL(b.Adla6,0) + ISNULL(b.Adla7,0) + ISNULL(b.Adla8,0) + ISNULL(b.Adla9,0) + ISNULL(b.Adla10,0) ELSE 0 END) Pay , SUM(CASE WHEN b.TransType = 'Return' THEN b.Amount ELSE 0 END) Rg ,
                      b.RefCode
								  ...
								  WHERE b.IsActive = 1 AND b.IsDeleted = 0 AND bm.IsDeleted=0 AND bm.IsActive=1
								    GROUP BY  b.RefCode
...
WHERE br.AccountId=@AccountId AND (br.BillAmt -ISNULL(selfa.Amount,0)-ISNULL(br.TdsAmt,0)  - ISNULL(adj.Rg,0) -ISNULL(adj.Pay,0) +ISNULL(bb.Pay,0) + ISNULL(bb.Rg,0) - ISNULL(br.RetAmt,0) - ISNULL(br.AdjustAmt,0)-ISNULL(bp.rcpt,0) > 0 OR CAST(bb.Amount AS NUMERIC(18))> 0)
