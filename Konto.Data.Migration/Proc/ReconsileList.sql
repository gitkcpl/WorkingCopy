...
from LedgerTrans l 
left outer join Acc Bac on Bac.Id=l.RefAccountId
left outer join Acc ac on ac.Id=l.AccountId
...
OR (l.VoucherDate <@FromDate and bt.BankDate BETWEEN @FromDate AND @ToDate)
)

and l.AccountId=@AccId
