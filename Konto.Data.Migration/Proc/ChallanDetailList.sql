...
BEGIN 
	...
	-- ct.Qty- ISNULL(b.qty,0) PendingQty,
	 ...
	 c.VoucherId,
	 c.IsDeleted,pc.ProcessName,
	  bf.BranchName FromBranch,bt.BranchName ToBranch,
	 c.Extra2,c.Extra1,p.Barcode
	 FROM Challan c
	 ...
	 left outer join Process pc on pc.Id = c.ProcessId
     LEFT OUTER JOIN color col ON col.Id=ct.ColorId
	 LEFT OUTER JOIN Branch AS bf ON bf.Id = c.BranchId
	 LEFT OUTER JOIN Branch AS bt ON bt.Id = c.ToBranchId
