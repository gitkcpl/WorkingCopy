...
BEGIN 
	  ...
	  bt.Qty,
	  bt.Rate, 
	  CAST( bt.Rate + ROUND((bt.Rate*(bt.CgstPer+bt.SgstPer+bt.IgstPer)/100),2) AS NUMERIC(18,2)) NetRate,
	  um.UnitName,
	  bt.Total GrossAmt,
	  bt.Disc DiscPer,
	  bt.DiscAmt,
      ...
      , bm.Id,
	  bm.IsDeleted, cc.HeadName CostCenter
 	  FROM BillMain bm
	  LEFT OUTER JOIN Acc ac on bm.AccId =ac.Id 
	  LEFT OUTER JOIN cost_heads cc on cc.Id = bm.CostHeadId
